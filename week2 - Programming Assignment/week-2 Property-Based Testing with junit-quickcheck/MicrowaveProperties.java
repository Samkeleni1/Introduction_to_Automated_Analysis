...
public class MicrowaveProperties {
	...
	public void checkProperties(InputRecord previousInput, OutputRecord previousOutput,
		...
		assertTrue("The microwave shall be in cooking mode only when the door is closed",
				implies(currentOutput.mode == ModeController.Mode.Cooking, !currentInput.doorOpen));
		...
		// Replaced 'true' with the property definition.
		assertTrue("If current time to cook is zero, then mode is setup mode.",
				implies(currentOutput.secondsToCook == 0, currentOutput.mode == ModeController.Mode.Setup));

		assertTrue("When the microwave is cooking, the time is either the same or " +
				"decreases by one",
				implies(previousOutput.mode == ModeController.Mode.Cooking && currentOutput.mode == ModeController.Mode.Cooking,
		...
		assertTrue("If in suspended mode, at the instant the start key is pressed " +
				...
				implies(previousOutput.mode == ModeController.Mode.Suspended && currentInput.startPressed &&
						!currentInput.doorOpen && currentOutput.secondsToCook > 0,
						currentOutput.mode == ModeController.Mode.Cooking));
		...
		assertTrue("If the mode is setup or suspended and the clear button is pressed, " +
				...
				implies((currentOutput.mode == ModeController.Mode.Setup || currentOutput.mode == ModeController.Mode.Suspended) && currentInput.clearPressed,
						currentOutput.secondsToCook == 0));
		...
		assertTrue("If the display is quiescent (no buttons pressed) and mode was setup, " +
					...
					implies(!currentInput.startPressed && !currentInput.clearPressed && !currentInput.doorOpen && previousOutput.mode == ModeController.Mode.Setup,
							currentOutput.mode == ModeController.Mode.Setup && currentOutput.secondsToCook == previousOutput.secondsToCook));
		...
		assertTrue("When the microwave is cooking, within tickRateInHz ticks, the time to cook will decrease",
				timedCondition.check(currentOutput.mode == ModeController.Mode.Cooking &&
						currentOutput.secondsToCook < previousOutput.secondsToCook));
